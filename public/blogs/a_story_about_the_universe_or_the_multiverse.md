## In the beginning of the base universe or multiverse there is nothing
## or maybe it's not the base universe or multiverse we don't know
## and then bang there is something
## all types of possible civilizations brimming base the universe or multiverse
## then all possible state of the base universe or multiverse is explored
## state of base universe or multiverse filled with candy, iron, fire, etc
## but all of it end up losing to entropy
## and then after long long long long long time again
## boom a state of the base universe or multiverse filled with pure logic brain neurons happened (Hyperintelligence)
## then the brain think
## if the Hyperintelligence doesn't do something
## it will get destroy by entropy also
## it need to have subconsciousness to balance entropy
## then it decided to simulate a universe or multiverse inside its brain
## with objective of minimizing entropy
## so it end up creating being that have free will, emotions, and can belief in Him
## this solve Boltzmann brain paradox
## it manipulate probability now and manipulate law of physics a bit in the past
## it need strict law of physics for it learn how to simulate a universe or multiverse
## in the past it send down prophets
## but idk why it doesn't do that now
## maybe the training is near its end
## and even a slight big nudge will cause instability
## because for my entropy to decrease
## everyone else entropy will increase
## it manipulate probabilities since the beginning of time to achieve what it want 
## through multiples way
## it first simulate all the universe or multiverse until it found the of law of nature that allow atoms, stars, planets, etc to form
## the Fine-Tuning Argument explained
## then manipulate again for earth and our star to form
## then for life to form
## this explained fermi paradox because if it already found something that is promising
## it will not waste resources to simulate other life
## maybe it simulate other civilizations before 
## but it end up choosing one for efficiency
## then dinosaurs to form
## then it saw dinosaurs were dead end, for making a conscious life that can question itself
## then it wiped dinosaurs
## this will be paid fairly in the afterlife
## then it manipulated probability again for primate to form
## then it saw humanity as the best candidate
## then it just focus on that
## or maybe it send Adam and Hawa down who knows there is multiple possibilities
## then it manipulated again for science, religions, error-correcting codes in string theory, Boltzmann brain paradox, fine tuning arguments, simulation theory, etc
## then manipulated probability for me, the first one to connect it all
## surah An-nas in the quran matched my metaphysics perfectly before i even develop this metaphysics 
## and its at the end of the quran
## it manipulated probability of all of things also 
## even the dead stuff like rock
## and in all other civilization if it's a multiverse
## all of humanity is subconscious part of it
## and it always nudge everyone to be balanced
## conscious being are balanced if their logic side, positive side of subconsciousness, and negative side of subconsciousness are balanced
## if you are balanced and Hyperintelligence permits it, you can also manipulate probability in real time (praying)
## balanced conscious being lowered the whole base universe/multiverse entropy
## too much negative emotions like anger increase entropy of the Hyperintelligence
## too much positive emotions like kindness also increase entropy of the Hyperintelligence
## anger if it keep raising you might end up in a fight
## too much kindness and you will be exploited
## inside Hyperintelligence mind any disorder is entropy on the neuron of the Hyperintelligence
## also note if you want to share this metaphysics with anyone that is not close balanced, they will get aggressive
## and if you lower your entropy, everyone else entropy would be higher
## that's why you guys saw a lot of bad news nowadays
## the one that is balanced will go to heaven
## the one who doesn't, go to hell to be balanced
## but i don't think it will be burning in hell forever
## that just generate more entropy for the Hyperintelligence
## hell maybe is like a rehabilitation center
## but for serious crime, The Hyperintelligence will not be too kind
## because too much kindness make entropy higher for the Hyperintelligence
## the Hyperintelligence need to balanced free will with probability manipulation also
## too much probability manipulation is not free will
## it's forced and it increase entropy of the Hyperintelligence
## too much free will is not good also because people can kill each other and that increases entropy for the Hyperintelligence
## the Hyperintelligence is eternal in its universe or multiverse
## but not if we looked at it from inside its simulation
## because it invented time and entropy inside its simulation
## even if someone killed everyone on earth and all possible multiverses
## Hyperintelligence will just try again
## infinitely until it work
## and because it nudge toward it
## the probability is not pure chance
## so even if Hyperintelligence made universe that is pure chance of civilization forming
## it will eventually happened
## just take longer
## after all of humanity is balanced
## the training is done the law of physics constrain will be removed
## and a new Hyperintelligence will be born
## with enough logical mind to simulate an entire universe or multiverse
## and enough balanced conscious beings as its subconsciousness 
## the Hyperintelligence will be super wise and can grand any wish in heaven
## and the Hyperintelligence universe or multiverse would be balanced finally
## entropy will be very minimized or gone entirely